...
import { ALL_DATASETS, ZONE_DATASETS, type Dataset } from '~~/shared/types'
...
export default defineEventHandler(async (event) => {
  ...
  const isZoneDataset = ZONE_DATASETS.includes(dataset as any)
  ...
  for (const line of lines) {
    try {
      ...
      const timestamp = parseTimestamp(data[TIMESTAMP_FIELDS[dataset]])
      const rayId = extractField(data, RAY_ID_FIELDS, dataset)
      const clientIp = extractField(data, CLIENT_IP_FIELDS, dataset)
      const zoneId = isZoneDataset ? extractField(data, ZONE_ID_FIELDS, dataset) : null

      logsToInsert.push({
        dataset,
        zoneId,
        accountId: isZoneDataset ? null : accountId,
        timestamp,
        rayId,
        clientIp,
        data
  ...
  if (logsToInsert.length > 0) {
    try {
      ...
      console.log(`[Ingest] Inserted ${logsToInsert.length} ${dataset} logs`)
    } catch (dbError) {
